...
import { Pie } from 'react-chartjs-2'; // Importer le graphique Pie
...
function Graphique() {
  ...
  const fetchAllData = async () => {
    ...
    try {
      do {
        const response = await fetch(`https://opendata.paris.fr/api/explore/v2.1/catalog/datasets/les-arbres/records?select=arrondissement&limit=${limit}&offset=${offset}`);
  ...
  useEffect(() => {
    ...
    fetchAllData().then(allData => {
      if (allData.length > 0) {
        ...
        const arrondissementCounts = allData.reduce((acc, record) => {
        ...
        const percentageData = Object.values(arrondissementCounts).map(count => (count / totalCount * 100).toFixed(2)); // Calculer les pourcentages
        setChartData({
          labels: Object.keys(arrondissementCounts), // Les arrondissements
          datasets: [
            {
              label: 'Pourcentage d\'arbres par arrondissement',
              ...
              backgroundColor: [
                  ...
                  'rgba(0, 255, 127, 0.6)', // Vert printemps
                  'rgba(152, 251, 152, 0.6)', // Vert pâle
                  'rgba(0, 100, 0, 0.6)', // Vert forêt
                  'rgba(124, 252, 0, 0.6)', // Vert chartreuse
                  'rgba(139, 69, 19, 0.6)', // Marron
    ...
    <div className='p-6'>
      {chartData ? (
        <div className='w-full max-w-lg mx-auto'> {/* Conteneur pour le graphique */}
          <Pie 
            data={chartData} // Remplacer Doughnut par Pie
            options={{
              responsive: true,
              plugins: {
                title: {
                  display: true, // Afficher le titre
                  text: 'Répartition des Arbres par Arrondissement', // Texte du titre
                  font: {
